...
function feetUpdate(dt)
	...
	local stepLength = clamp(velLength*1.5, 0.35, 1.4)
	local stepTime = math.min(stepLength / velLength * 0.4, 0.4)
...
function weaponsInit()
	...
	for i=1, #locs do
		...
		if hit then
			...
			if not weapon.chargeTime then weapon.chargeTime = 0.8 end
...
function weaponFire(weapon, pos, dir)
	...
	local extraSpread = math.min(0.01, 1.3 / robot.distToPlayer)
	...
	if weapon.type == "gun" then
		PlaySound(shootSound, pos, 0.6, false)
...
function hitByExplosion(strength, pos)
	...
	if strength > 0.99 then
		...
		if f > 0.2 then
			robot.stunned = math.max(robot.stunned, f * 999999.0)
		...
		if hit then
			for i=1, #robot.allShapes do
				if robot.allShapes[i] == shape then
					robot.stunned = robot.stunned + 900
					PlaySound(deathSound, robot.bodyCenter, 100.0, false)
					return
				end
			end
