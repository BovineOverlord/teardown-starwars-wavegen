...
#include "faction.lua"
FACTION = "empire"		-- Star Wars faction: empire | rebel
UNIT_HERO = true		-- hero: capped to 1 alive per side
...
function robotUpdate(dt)
	...
	--Star Wars: target the nearest hostile (enemy-faction unit or the player)
	local pp = factionUpdateTargeting(dt)
...
function feetUpdate(dt)
	...
	local stepLength = clamp(velLength*1.5, 0.35, 1)
	local stepTime = math.min(stepLength / velLength * 0.6, 0.6)
	local stepHeight = stepLength * 0.2
...
function weaponsInit()
	...
	for i=1, #locs do
		...
		if hit then
			...
			if not weapon.chargeTime then weapon.chargeTime = 1 end
			if not weapon.fireCooldown then weapon.fireCooldown = 0.1 end
...
function weaponEmitFire(weapon, t, amount)
	...
	local d = TransformToParentVec(t, Vec(0, 0, -2))
	ParticleReset()
	ParticleType("smoke")
	ParticleTile(1)
	ParticleColor(0.6, 0.55, 1, 0.15, 0.1, 1)
	ParticleRadius(0, 9*amount)
	ParticleEmissive(9, 0)
	ParticleStretch(10, 0)
	ParticleAlpha(1.0, 0.1)
	ParticleDrag(0.1)
	ParticleCollide(0.1)
	ParticleGravity(math.random()*5)
	PointLight(p, 0.25, 0.2, 0.8, 1*amount)
	PlayLoop(fireLoop, t.pos, amount)
	PlayLoop(fireLoop2, t.pos, amount)
	PlayLoop(fireLoop3, t.pos, amount)
	SpawnParticle(p, VecScale(d, 9), 0.15*amount)
	...
	if amount > 0.5 then
		...
		if spawnFireTimer > 0 then
			...
			rejectAllBodies(robot.allBodies)
			local hit, dist = QueryRaycast(p, d, 1)
			if hit then
		...
		--Hurt player
		--Star Wars: lightsaber / force power also strikes down nearby enemy units
		if faction and not faction.targetIsPlayer and faction.target ~= 0
			and IsHandleValid(faction.target) and robot.distToPlayer < 4.5 then
			SetTag(faction.target, "sw_kill")
		end
	...
	for i=1, #weapons do
		...
		if weapon.type == "fire" then
			...
			if head.canSeePlayer and gotAim and robot.distToPlayer < weapon.maxDist then
				QueryRequire("physical large")
				rejectAllBodies(robot.allBodies)
				if faction and not faction.targetIsPlayer then
					clearShot = factionLosClear(t.pos, fwd, distToPlayer)
				else
	...
	if config.hasVision and config.canSeePlayer then
		if distToPlayer < config.viewDistance then	--Within view distance
			...
			if VecDot(toPlayer, fwd) > limit then --In view frustum
				...
				local losClear
				if faction and not faction.targetIsPlayer then
					losClear = factionLosClear(et.pos, toPlayer, distToPlayer)
				else
					losClear = not QueryRaycast(et.pos, toPlayer, distToPlayer, 0, true)
				end
				if losClear then --Not blocked
					playerVisible = true
				end
			end
		end
...
function hitByExplosion(strength, pos)
	...
	if strength > 0.99 then
		...
		if f > 0.2 then
			robot.stunned = math.max(robot.stunned, f * 5.0)
			--PlaySound(deathSound, robot.bodyCenter, 100.0, false)
		end
