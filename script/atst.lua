...
UNIT_WEIGHT = 5			-- heavy walker: counts as 5 units
...
function hoverInit()
	...
	if #f > 0 then
		...
		if hit then
			...
			hover.distPadding = math.min(0.3, dist*30)
		end
	end
end
...
function hoverUpdate(dt)
	...
	if hit then
		...
		hover.contact = clamp(1.0 - (dist - hover.distTarget) / hover.distPadding, 0.0, 10.0)
...
function feetUpdate(dt)
	...
	local stepLength = clamp(velLength*1.5, 0.35, 9.9)
	...
	for i=1, #feet do
		...
		if hover.contact > 0 then
			if foot.stepAge < foot.stepLifeTime then
				...
				if foot.stepAge == foot.stepLifeTime then
					PlaySound(stepSound, p, 0.9, false)
				end
			end
...
function weaponsInit()
	...
	for i=1, #locs do
		...
		if hit then
			...
			if not weapon.shotsPerRound then weapon.shotsPerRound = 2 end
...
function weaponFire(weapon, pos, dir)
	...
	pos = VecAdd(pos, VecScale(dir, 1.6))
	pos2 = VecAdd(pos, VecScale(dir, 10))
	pos3 = VecAdd(pos, VecScale(dir, 20))
	pos4 = VecAdd(pos, VecScale(dir, 30))
	pos5 = VecAdd(pos, VecScale(dir, 40))		
	
	if weapon.type == "gun" then
		PlaySound(shootSound, pos, 0.25, false)
		PointLight(pos, 1, 0.01, 0.01, 200)
		PointLight(pos2, 1, 0.01, 0.01, 170)
		PointLight(pos3, 1, 0.01, 0.01, 150)
		PointLight(pos4, 1, 0.01, 0.01, 100)
		PointLight(pos5, 1, 0.01, 0.01, 50)			
...
function navigationSetTarget(pos, timeout)
	...
	if VecDist(navigation.target, pos) > 5.0 then
...
function hitByExplosion(strength, pos)
	...
	if strength > 0.99 then
		...
		if f > 0.2 then
			robot.stunned = math.max(robot.stunned, f * 99999.0)
		end
