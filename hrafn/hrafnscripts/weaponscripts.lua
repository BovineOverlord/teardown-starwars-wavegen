...
function initAutoCannon()
	...
	autoCannonSound2 = LoadSound("MOD/snd/blaster.ogg", 25.0)
    autoCannonSound3 = LoadSound("MOD/snd/l0.ogg", 10.0)	
	hasAutoCannon = autoCannonSettings ~= 0
	autoCannonRecoil = 1
	autoCannonDmg = 1
	autoCannonFR = 50/660 --fire rate
	autoCannonShotCounter = {2,5}
...
function autoCannonFire()
	...
	--PointLight(origin.pos, 0.9, 0.1, 0.1, 90)
	PlaySound(autoCannonSound,origin.pos,0.7,false)
	PlaySound(autoCannonSound2,origin.pos,0.3,false)	
...
function autoCannonShellOperation(shell)
	...
	if hit then
		...
		PlaySound(autoCannonSound3,hitPos,0.5,false)
		...
		PointLight(ahead, 1, 0.1, 0.08, 30)
